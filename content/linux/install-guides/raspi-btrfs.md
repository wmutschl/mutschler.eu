...
Now as everything is up and running from `/dev/mapper/crypt_raspi` on `/dev/sda3`, we don't have any use for `/dev/sda2` anymore. So, either let it be, delete it, or use it as an [encrypted swap partition](../ubuntu-btrfs/#encrypted-swap) or (what I do) as an [encrypted docker image partition](../raspi-post-install/#dedicated-docker-image-partition).
