...
```
Either log out and back in again, or restart your system, to ensure snap’s paths are updated correctly. Check whether there are any updates:
```bash
sudo snap refresh
```
...
### Browser
I am using Firefox for almost all of my browsing which is installed by default. I am using the following:
...
#### Hugo
My website uses Hugo, so I install it from the repos:
```bash
